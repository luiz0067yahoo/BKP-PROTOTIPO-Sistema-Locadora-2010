...
;ZeroClipboard.setMoviePath( '/sites/all/modules/exampledepot/zeroclipboard/ZeroClipboard.swf' );

onload = function() {
	var code_blocks = getElementsByClassName('exd_code');
	for (var i=0; i<code_blocks.length; i++) {
		code_head = getElementsByClassName('exd_code_head', code_blocks[i])[0];
		code_a = code_head.getElementsByTagName('a')[0];

		// Create a clipboard client on the <a> element and watch for mouse down events
		var clip = new ZeroClipboard.Client();
		clip.glue(code_a);
		clip.setText("test");
		clip.body = getElementsByClassName('exd_code_body', code_blocks[i])[0];
		clip.addEventListener('mouseDown', function(client) { 
			var code = client.body.innerHTML.replace(/<\/?[^>]+(>|$)/g, ''); 
...
function  artNoStyleAdding(parent) {return;
